...
@Component({
  selector: 'app-mis-daily-acitiy-headsummary-consultant',
  templateUrl: './mis-daily-acitiy-headsummary-consultant.component.html',
  styleUrls: ['./mis-daily-acitiy-headsummary-consultant.component.css']
})
export class MisDailyAcitiyHeadsummaryConsultantComponent {
  uname = '';
    ...
    this._studentservice.misdailyacticity_conssultant(routerParams["vrdt1"], routerParams["vrdt2"], routerParams["doc1"])
      .subscribe((data: Students[]) => {
        ...
        for (let i = 0; i < this.Students.length; i++) {
          ...
          this.dcmtTypes.add(this.Students[i].Reporttype.toString());

          let headtotal = this.finalTotal.get(this.Students[i].Reporttype.toString());
          if(headtotal != null && headtotal != undefined){
            this.finalTotal.set(this.Students[i].Reporttype.toString(),Number(headtotal) + Number(this.Students[i].recamt))
          } else {
  ...
  sortDcmt() {
    return Array.from(this.dcmtTypes).sort()
  }

  checkIfNull(doc: String, dcmtType: String, head: String) {
    let d = this.typewiseTotal.get(doc)
    if (d != null && d != undefined) {
      let dt = d.get(dcmtType)
